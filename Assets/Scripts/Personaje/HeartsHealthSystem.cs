...
public class HeartsHealthSystem{
    ...
    private List<Heart> heartList = null;
    ...
    {
        //Cycle through all hearts starting from the end -> Revisa si el corazon puede tolerar el da�o recivido en base a los fragmentso de este (max 4 minimo 1)
        for (int i=heartList.Count - 1; i >= 0; i--)
        {
            Heart heart = heartList[i];
            //Test if this heart can absorb demageAmount
            if(damageAmount > heart.GetFragmentAmount())
            {
                //Heart cannot absorb full damageAmount, damage heart and keep going to next
